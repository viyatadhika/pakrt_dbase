...
<header class="sticky-header px-5 py-4 relative">
    <div class="flex items-center gap-3 min-w-0">
        ...
        <div class="min-w-0">
            <h1 class="text-[17px] font-extrabold text-sky-600 leading-tight truncate">Stok Barang</h1>
            <p class="text-[12px] text-gray-400 font-medium leading-tight">Lihat dan kelola stok barang</p>
        </div>
    </div>
...
<!-- Search + Filter Tabs (fixed di bawah header) -->
...
<!-- Main List -->
<main id="listBarang" class="px-4 py-4 mb-28 bg-white" style="margin-top:73px;">
    ...
    <div id="emptyState" class="<?= ($totalData == 0 ? 'flex' : 'hidden') ?> flex-col items-center justify-center py-20 px-6 text-center">
        ...
        <p class="text-gray-400 text-xs mt-2 leading-relaxed">Silakan tambah barang baru atau ubah kriteria pencarian.</p>
            ...
            <form id="mainForm" onsubmit="event.preventDefault(); handleSimpan();" class="space-y-3">
                ...
                <div>
                    ...
                    <input type="text" id="inpKode"
                        class="w-full mt-1 px-4 py-3 rounded-2xl bg-gray-100 border border-gray-200 text-gray-500 font-bold text-sm" readonly>
                </div>
                <div>
...
<script>
    ...
    window.addEventListener('load', function() {
        setTimeout(function() {
            ...
            var main = document.getElementById('listBarang');
    ...
    function enableEditMode() {
        if (!CAN_EDIT || !currentDetailData?.id) return;
        sheetMode = 'edit';
    ...
    function ensureKategoriSection(kategoriName) {
        ...
        sec.innerHTML = `<div class="flex items-center gap-2 mb-4"><div class="h-5 w-1.5 bg-sky-500 rounded-full"></div><h2 class="font-bold text-gray-800 text-sm kategoriTitle">${kategoriName}</h2></div><div class="space-y-3 kategoriList"></div>`;
        ...
        fetch(url, {
            ...
            .then(res => {
                ...
                if (isEditMode) {
                    ...
                    const oldKatId = oldCard ? oldCard.getAttribute('data-kategori-id') : null;
                    updateCardDom(item);
                    if (oldCard && oldKatId !== String(item.kategori_id)) {
                        const sec = ensureKategoriSection(getKategoriNameById(item.kategori_id));
                        sec.querySelector('.kategoriList')?.prepend(oldCard);
                    }
                    ...
                    sec.querySelector('.kategoriList')?.prepend(card);
                    emptyState.classList.add('hidden');
                    emptyState.classList.remove('flex');
                    if (sectionsWrapper) sectionsWrapper.style.display = 'block';
    ...
    function handleHapus() {
        if (!CAN_EDIT) return;
        const id = document.getElementById('inpId').value;
        if (!id || !confirm('Hapus barang ini?')) return;
        ...
        fetch('stok_barang_hapus.php', {
            ...
            .then(res => {
                ...
                if (card) {
                    const sec = card.closest('.kategori-section');
                    card.remove();
                    if (sec && sec.querySelector('.kategoriList').children.length === 0) sec.remove();
                }
                closeStokModal();
                updateDisplay(mainSearch.value);
