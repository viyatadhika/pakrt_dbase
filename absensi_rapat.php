...
{
    if ($startDate === $endDate) return formatTanggalID($selectedDate);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $canAttend) {
    ...
    if ($namaPeserta === '') {
        ...
        $cek = $conn->prepare("
            ...
            WHERE booking_id = ? AND tanggal_hadir = ?
              AND LOWER(TRIM(nama_peserta)) = LOWER(TRIM(?))
            LIMIT 1
        ");
        $dup = null;
        if ($cek) {
            $cek->bind_param('iss', $bookingId, $attendanceDate, $namaPeserta);
            $cek->execute();
        ...
        if ($dup) {
            $error = 'Nama "' . h($namaPeserta) . '" sudah tercatat hadir untuk sesi ini.';
        } else {
            $stmt = $conn->prepare("
...
<style>
    ...
    *::after {
    ...
    .btn-absen-lagi {
        ...
        animation: fadeUp .3s .40s ease both
    }

    .btn-absen-lagi:hover {
        background: var(--blue-lt)
    }
</style>
...
<?php if ($isBeforeStart): ?>
    ...
    <div class="done-wrap">
        ...
        <h1 style="font-size:22px;font-weight:800;letter-spacing:-.02em;margin-bottom:10px;animation:fadeUp .3s .12s ease both">Absensi Belum Dibuka</h1>
        <p style="font-size:14px;color:var(--muted);line-height:1.7;max-width:320px;animation:fadeUp .3s .2s ease both">
            Absensi baru bisa dilakukan mulai tanggal <strong style="color:var(--ink)"><?= h(formatTanggalID($booking['start_date'])) ?></strong>.
        </p>
    </div>
...
<?php elseif ($success): ?>
    ...
    <div class="success-wrap">
        ...
        <a href="absensi.php?id=<?= $bookingId ?>" class="btn-absen-lagi">
            <i class="fa-solid fa-user-plus"></i> Peserta Berikutnya
        </a>
    </div>
...
<script>
    ...
    if (canvas) {
        ...
        function onMove(e) {
            ...
            ctx.stroke();
            ctx.beginPath();
            ctx.moveTo((lastX + p.x) / 2, (lastY + p.y) / 2);
            lastX = p.x;
            lastY = p.y;
        }
