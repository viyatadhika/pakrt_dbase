        ...
        <div class="w-full max-w-md bg-white rounded-3xl p-5 shadow-xl">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <?php endif; ?>
                    <button type="button" onclick="closeModal()" class="w-9 h-9 rounded-full bg-gray-100 text-gray-600 flex items-center justify-center">
                        <i class="fa-solid fa-xmark"></i>
                    </button>
                </div>
            </div>

            <!-- Form — gunakan class untuk kontrol readonly, BUKAN attribute disabled -->
            <form id="agenda-form" onsubmit="handleSave(event)" class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input id="f-peserta" type="number"
                            class="w-full mt-1 px-4 py-3 rounded-2xl bg-gray-50 border border-gray-200">
                    </div>
                    <div>
                        <label class="text-xs font-bold text-gray-600">Asrama</label>
                        <input id="f-asrama" type="text"
                            class="w-full mt-1 px-4 py-3 rounded-2xl bg-gray-50 border border-gray-200">
                    </div>
                </div>
                <div class="grid grid-cols-2 gap-3">
...
<script>
    ...
    // ─── FIELD IDs ───────────────────────────────────────────────
    const FIELD_IDS = ['f-judul', 'f-start', 'f-end', 'f-pny', 'f-asrama', 'f-peserta', 'f-kelas', 'f-makan'];
    ...
    // ─── READONLY via CSS class (BUKAN disabled) ─────────────────
    ...
    function renderCalendar() {
        ...
        const currentEvents = agendaData.filter(ev => {
            const s = new Date(ev.start),
                e = new Date(ev.end);
            return (s.getMonth() === month || e.getMonth() === month) && s.getFullYear() === year;
        });

        document.getElementById('view-title').innerText = "Daftar Agenda";
        document.getElementById('badge-count').innerText = `${countAgendaBulan(agendaData, viewDate)} Agenda Bulan Ini`;
    ...
    function markBentrok(data) {
        return data.map((a, i) => ({
            ...a,
            isBentrok: data.some((b, j) => i !== j && a.start <= b.end && a.end >= b.start)
        }));
    }
    ...
    function groupBentrok(data) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            for (let j = i + 1; j < data.length; j++) {
                ...
                if (group.some(g => data[j].start <= g.end && data[j].end >= g.start)) {
                    group.push(data[j]);
                    used.add(data[j].id);
                }
            }
    ...
    function countAgendaBulan(data, vd) {
    ...
    function renderList(data) {
        ...
        document.getElementById('badge-count').innerText = `${data.length} Agenda`;
        ...
        <div class="space-y-3">
            ...
            <div class="space-y-3 ${isBentrok ? 'p-3 bg-red-50/30 rounded-[2.5rem] border border-red-100/50' : ''}">
                ${group.map(item => {
                    ...
                    const badgeKat     = `<span class="text-[9px] font-black uppercase px-2 py-1 rounded-lg ${colorMap[item.pny]} bg-opacity-10 flex-shrink-0" style="color:${getHexColor(item.pny)}">${item.pny}</span>`;
                    return `
                    <div onclick="openModalDetail(${item.id})"
                        ...
                        <div class="flex-1 overflow-hidden">
                            <div class="flex justify-between items-start mb-0.5">
                                ...
                                <div class="flex flex-col items-end gap-1 flex-shrink-0">${badgeSelesai}${badgeKat}</div>
                            </div>
    ...
    function openModalTambah() {
        if (!IS_ADMIN) return;
        document.getElementById('sheetTitle').innerText = "Tambah Jadwal";
        ...
        elAdmin('btnEditTrigger', el => el.classList.add('hidden'));
        elAdmin('btnHapus', el => el.classList.add('hidden'));
        elAdmin('btnSubmit', el => {
            el.classList.remove('hidden');
            el.innerText = "Simpan Jadwal";
        });
    ...
    function openModalDetail(id) {
        ...
        document.getElementById('sheetTitle').innerText = "Detail Jadwal";
        document.getElementById('edit-id').value = item.id;
        setVal('f-judul', item.judul);
        setVal('f-start', item.start);
        setVal('f-end', item.end);
        setVal('f-pny', item.pny);
        setVal('f-asrama', item.asrama);
        setVal('f-peserta', item.peserta ?? '');
        setVal('f-kelas', item.kelas ?? '');
        setVal('f-makan', item.makan ?? '');

        // Mode detail: form read-only via CSS
        setFormReadOnly(true);

        elAdmin('btnEditTrigger', el => el.classList.remove('hidden'));
        elAdmin('btnHapus', el => el.classList.add('hidden'));
        elAdmin('btnSubmit', el => el.classList.add('hidden'));
    ...
    function enableEditMode() {
        if (!IS_ADMIN) return;
        document.getElementById('sheetTitle').innerText = "Ubah Jadwal";

        // Buka form untuk diedit
        setFormReadOnly(false);

        elAdmin('btnEditTrigger', el => el.classList.add('hidden'));
        elAdmin('btnHapus', el => el.classList.remove('hidden'));
        elAdmin('btnSubmit', el => {
            el.classList.remove('hidden');
            el.innerText = "Simpan Perubahan";
        });
    }
    ...
    async function handleSave(e) {
        ...
        // Pastikan form tidak dalam state readonly sebelum ambil nilai
        setFormReadOnly(false);

        const id = getVal('edit-id');
        const judul = getVal('f-judul').trim();
        const start = getVal('f-start');
        const end = getVal('f-end');
        const pny = getVal('f-pny');
        const asrama = getVal('f-asrama').trim();
        const peserta = getVal('f-peserta');
        const kelas = getVal('f-kelas').trim();
        const makan = getVal('f-makan').trim();

        if (!judul || !start || !end) {
            showToast('Nama, tanggal mulai & selesai wajib diisi');
            return;
        }
        if (start > end) {
        ...
        fd.append('end', end);
        fd.append('pny', pny);
        fd.append('asrama', asrama);
        fd.append('peserta', peserta);
        fd.append('kelas', kelas);
        fd.append('makan', makan);
        ...
        try {
            ...
            if (result.error) {
                ...
                if (btnSubmit) {
                    ...
                    btnSubmit.innerText = id ? "Simpan Perubahan" : "Simpan Jadwal";
                }
                return;
            }

            closeModal();
            showToast(id ? '✓ Jadwal berhasil diubah' : '✓ Jadwal berhasil disimpan');
            loadAgenda();
        } catch (err) {
    ...
    function filterByDate(dateStr) {
        const events = agendaData.filter(ev => dateStr >= ev.start && dateStr <= ev.end);
        renderList(events);
        document.getElementById('view-title').innerText = dateStr.split('-').reverse().join(' / ');
        document.getElementById('badge-count').innerText = `${events.length} Agenda`;
