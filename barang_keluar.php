...
<header class="sticky-header px-5 py-4 relative">
    <div class="flex items-center gap-3 min-w-0">
        <button onclick="window.history.back()"
            class="w-10 h-10 shrink-0 flex items-center justify-center rounded-full bg-orange-50 text-orange-600 hover:bg-orange-100 transition">
    ...
    <button onclick="openExportModal()"
        class="absolute top-5 right-4 w-11 h-11 flex items-center justify-center text-orange-600 hover:bg-orange-50 rounded-full transition text-lg">
    ...
    <div class="px-4 pt-3 pb-3">
        <div class="relative group">
            ...
            <input type="text" id="searchTransaksi"
                placeholder="Cari ref / penerima / barang / kategori..."
                class="w-full pl-11 pr-4 py-3 bg-gray-50 border border-transparent rounded-2xl text-sm focus:bg-white focus:border-orange-300 outline-none transition-all">
        </div>
    </div>
</div>
...
<!-- FAB -->
<?php if ($canEdit): ?>
    <a href="barang_keluar_tambah.php"
        class="fixed bottom-8 right-8 w-11 h-11 bg-orange-600 text-white rounded-full shadow-lg shadow-orange-100 flex items-center justify-center z-[40] active:scale-90 transition-all">
...
<div id="sheetDetail" class="fixed bottom-0 left-0 right-0 bg-white rounded-t-[28px] z-[60] translate-y-full transition-transform duration-300">
    ...
    <div class="px-6 pb-4 border-b border-gray-100">
        <div class="flex items-center justify-between">
            <div>
                <h3 class="font-extrabold text-gray-800 text-sm">Detail Barang Keluar</h3>
                <p id="sheetRef" class="text-[11px] text-orange-600 font-bold">-</p>
            </div>
        ...
        <div class="w-full max-w-md bg-white rounded-3xl p-5 shadow-xl">
            ...
            <div class="space-y-3">
                <div>
                    ...
                    <input type="date" id="exportFrom" class="w-full mt-1 px-4 py-3 rounded-2xl bg-gray-50 border border-gray-200 outline-none focus:border-orange-300">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-600">Sampai Tanggal</label>
                    <input type="date" id="exportTo" class="w-full mt-1 px-4 py-3 rounded-2xl bg-gray-50 border border-gray-200 outline-none focus:border-orange-300">
                </div>
                <button onclick="downloadExport()" class="w-full py-3 rounded-2xl bg-orange-600 text-white font-extrabold text-sm">Download PDF</button>
...
<script>
    ...
    function downloadExport() {
        ...
        window.location.href = "barang_keluar_export.php" + (p.length ? "?" + p.join("&") : "");
    }
    ...
    function openSheetLocal(el) {
        ...
        const penerima = el.dataset.penerima || '-';
        ...
        let html = `
            <div class="text-xs text-gray-500 space-y-1">
                ...
                <div class="flex justify-between"><span class="text-gray-400">Penerima</span><span class="font-semibold text-gray-700">${penerima}</span></div>
                ...
                html += `
                    <div class="flex justify-between items-center py-2 border-b border-gray-100">
                        ...
                        <div class="text-right ml-3 shrink-0">
                            <p class="text-sm font-bold text-red-600">- ${i.qty ?? 0}</p>
    ...
    function applySearchFilter() {
        ...
        document.querySelectorAll('.groupTanggal').forEach(group => {
            ...
            group.querySelectorAll('.transaksi-item').forEach(item => {
                ...
                const penerima = (item.dataset.penerima || '').toLowerCase();
                ...
                const cocok = keyword === '' || ref.includes(keyword) || ringkasan.includes(keyword) ||
                    ...
                    penerima.includes(keyword) || ket.includes(keyword) ||
    ...
    async function refreshList() {
        try {
            const res = await fetch('barang_keluar_list_ajax.php', {
                cache: 'no-store'
            });
            if (!res.ok) return;
