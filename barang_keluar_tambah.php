...
<style>
    ...
    .input-field:focus {
        border-color: #fdba74;
        box-shadow: 0 0 0 4px rgba(255, 237, 213, 0.7);
    }
...
<div class="header-container border-b border-gray-50">
    <header class="max-w-4xl mx-auto px-4 py-4 flex items-center justify-between">
        <div class="flex items-center gap-4">
            <button onclick="window.history.back()" class="w-10 h-10 flex items-center justify-center rounded-full bg-orange-50 text-orange-600 hover:bg-orange-100 transition">
                <i class="fa-solid fa-arrow-left"></i>
            </button>
            <div>
                <h1 class="text-lg font-extrabold text-orange-600 leading-tight">Barang Keluar</h1>
                <p class="text-[12px] text-gray-500">Input barang keluar gudang</p>
            </div>
        </div>
        <div class="flex items-center">
            <p class="text-[10px] sm:text-[11px] font-semibold text-orange-700 bg-orange-50 px-3 sm:px-4 py-1.5 sm:py-2 rounded-full border border-orange-100">
    ...
    <form id="formBarangKeluar" onsubmit="event.preventDefault();">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
            ...
            <div class="space-y-6">
                ...
                <!-- Data Permintaan -->
                <div class="card-custom p-6 bg-white border-orange-50 bg-orange-50/5">
                    ...
                    <div class="space-y-5">
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1.5 ml-1">Peminta / Divisi</label>
                            <div class="input-with-icon">
                                <input type="text" id="inpTujuan" required placeholder="Nama atau Divisi"
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1.5 ml-1">Tanggal Keluar</label>
                            <div class="input-with-icon">
                        ...
                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1.5 ml-1">Keterangan (opsional)</label>
                            <div class="input-with-icon">
                                <textarea id="inpKet" rows="2" placeholder="Misal: untuk kebutuhan rapat / divisi ..."
                ...
                <!-- Pilih Barang -->
                <div class="card-custom p-6 bg-white border-orange-100">
                    <div class="flex items-center gap-3 mb-6">
                    ...
                    <div class="space-y-5">
                        <div>
                            ...
                            <p id="stokHint" class="hidden text-[10px] mt-2 ml-1 text-orange-600 font-bold uppercase tracking-tight">
                                Tersedia: <span id="stokVal">0</span>
                            </p>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1.5 ml-1">Jumlah Keluar</label>
                            <div class="input-with-icon">
                                <input type="number" id="inpQty" placeholder="0"
                                    class="input-field w-full px-4 py-3 bg-orange-50 border-orange-200 rounded-xl outline-none transition text-sm font-bold text-orange-700">
                                <i class="fa-solid fa-minus-circle"></i>
                            </div>
                        </div>

                        <div>
                            <label class="block text-xs font-semibold text-gray-600 mb-1.5 ml-1">Catatan Item (opsional)</label>
                            <div class="input-with-icon">
                                <textarea id="inpNote" placeholder="Misal: urgent / untuk ATK rapat ..." rows="2"
                                    class="input-field w-full px-4 py-3 bg-gray-50 border border-transparent rounded-xl outline-none transition text-sm"></textarea>
                                <i class="fa-solid fa-comment-dots"></i>
                            </div>
                        </div>
                        ...
                        <button type="button" onclick="addItem()"
                            class="btn-orange w-full h-[46px] text-white rounded-xl font-bold text-xs shadow-lg shadow-orange-100">
                            Tambahkan ke Daftar
                        </button>
                    </div>
                </div>
                ...
                <div class="card-custom min-h-[400px] flex flex-col bg-white">
                    ...
                    <div id="cartList" class="flex-1 p-4 sm:p-6 space-y-3 custom-scrollbar overflow-y-auto max-h-[520px]">
                        <div class="h-full flex flex-col items-center justify-center text-gray-300 py-10 text-center">
                            <i class="fa-solid fa-cart-flatbed-suitcases text-4xl mb-3 opacity-20"></i>
                            <p class="text-xs font-medium px-6">Belum ada barang dipilih. Gunakan form di sebelah kiri.</p>
                        </div>
                    </div>
                    ...
                    <div id="cartFooter" class="hidden p-6 bg-slate-50/50 rounded-b-[1.5rem] border-t border-gray-100">
                        <button type="button" id="btnSimpan" onclick="simpanTransaksi()"
                            class="btn-orange w-full py-4 text-white rounded-xl font-bold text-sm shadow-xl shadow-orange-100">
...
<script>
    ...
    function addItem() {
        ...
        const opt = sel.options[sel.selectedIndex];
        const stokTersedia = parseInt(opt.getAttribute('data-stok')) || 0;
        const kodeBarang = sel.value;
        const existingItem = state.items.find(item => item.kode === kodeBarang);
        const currentTotal = (existingItem ? existingItem.qty : 0) + qty;

        if (currentTotal > stokTersedia) {
            alert('Stok tidak mencukupi! Tersedia hanya ' + stokTersedia + ' ' + opt.getAttribute('data-unit'));
            return;
        }

        if (existingItem) {
            existingItem.qty = currentTotal;
            ...
            list.innerHTML = state.items.map((item, index) => `
                <div class="bg-white p-4 rounded-xl border border-gray-100 flex flex-col gap-2 shadow-sm relative overflow-hidden">
                    ...
                    <div class="flex justify-between items-center bg-slate-50 p-2 rounded-lg mt-1">
                        <div class="pl-2">
                            <p class="text-[8px] text-gray-400 font-bold uppercase tracking-widest">Jumlah Keluar</p>
                            <p class="text-sm font-black text-orange-700">${item.qty}
                                <span class="text-[10px] font-bold text-gray-400 ml-1">${item.unit}</span>
                            </p>
                        </div>
                        <div class="text-right pr-2">
                            <p class="text-[9px] italic text-slate-500 truncate max-w-[160px]">"${item.note}"</p>
                        </div>
                    </div>
                </div>
            `).join('');
    ...
    async function simpanTransaksi() {
        ...
        if (!penerima) return alert('Isi peminta/divisi dulu');
        if (!tanggal) return alert('Tanggal wajib diisi');
        if (!state.items.length) return alert('Belum ada barang dipilih');
